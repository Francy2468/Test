...
bj = function(aQ, bO, bw)
    ...
    bP.GetPlayerFromCharacter = function(self, ck)
        -- eUNC passes a plain {} table — should return nil, not a proxy
    ...
    bP.HttpGet = function(self, cI, isAsync)
        local cL = aE(cI)
        table.insert(t.string_refs, {value = cL, hint = "HTTP URL"})
        t.last_http_url = cL
        local _body = _fetch_url(cL)
        if _body then return _body end
        return cL
    end
...
-- Shared URL fetcher used by both game:HttpGet and exploit httpget
local function _fetch_url(url)
    if type(url) ~= "string" or not url:match("^https?://") then return nil end
    local _cmds = {
        'curl -sL --max-time 15 --compressed -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36" "' .. url:gsub('"','\\"') .. '" 2>/dev/null',
        'curl -sL --max-time 15 "' .. url:gsub('"','\\"') .. '" 2>/dev/null',
        'wget -qO- --timeout=15 "' .. url:gsub('"','\\"') .. '" 2>/dev/null',
    }
...
loadstring = function(al, eu)
    ...
    if ev then
        -- Try to fetch and execute real content first
        if cI:match("^https?://") then
            local _fetched = _fetch_url(cI)
            if _fetched and #_fetched > 10 and not _fetched:match("^https?://") then
                local _src = _fetched
                if type(_src) == "string" and #_src > 0 and _src:byte(1) ~= 0x1b then
                    _src = I(_src)
                end
                local _fn2, _err2 = e(_src)
                if _fn2 then return _fn2 end
            end
        end
    ...
    if cI:match("^https?://") then
        -- Try to actually fetch and execute the URL content
        local _fetched = _fetch_url(cI)
        if _fetched and #_fetched > 10 and not _fetched:match("^https?://") then
            -- Got real content — sanitise and compile it
            local _src = _fetched
            if type(_src) == "string" and #_src > 0 and _src:byte(1) ~= 0x1b then
                _src = I(_src)
            end
            local _fn, _err = e(_src)
            if _fn then
                -- Mark so we don't re-fetch on recursive calls
                if not t._loadstring_seen.ok then t._loadstring_seen.ok = {} end
                local _key = tostring(#_src) .. ":" .. _src:sub(1,32)
                t._loadstring_seen.ok[_key] = true
                return _fn
            end
        end
        -- Fetch failed or produced no usable code — emit a comment and return stub
...
function q.dump_file(eN, eO)
    ...
    rawset(eR, "game",        game)
    rawset(eR, "Game",        game)
    rawset(eR, "workspace",   workspace)
    rawset(eR, "Workspace",   workspace)
    rawset(eR, "script",      script)
    rawset(eR, "Enum",        Enum)
    rawset(eR, "Instance",    Instance)
    rawset(eR, "object",      object)
    rawset(eR, "shared",      shared)
    rawset(eR, "SharedTable", SharedTable)
    -- ── Roblox type constructors ────────────────────────────────────────────
    rawset(eR, "Vector3",                  Vector3)
    rawset(eR, "Vector2",                  Vector2)
    rawset(eR, "Vector3int16",             Vector3int16)
    rawset(eR, "Vector2int16",             Vector2int16)
    rawset(eR, "CFrame",                   CFrame)
    rawset(eR, "Color3",                   Color3)
    rawset(eR, "BrickColor",               BrickColor)
    rawset(eR, "UDim",                     UDim)
    rawset(eR, "UDim2",                    UDim2)
    rawset(eR, "TweenInfo",                TweenInfo)
    rawset(eR, "NumberRange",              NumberRange)
    rawset(eR, "NumberSequence",           NumberSequence)
    rawset(eR, "NumberSequenceKeypoint",   NumberSequenceKeypoint)
    rawset(eR, "ColorSequence",            ColorSequence)
    rawset(eR, "ColorSequenceKeypoint",    ColorSequenceKeypoint)
    rawset(eR, "Rect",                     Rect)
    rawset(eR, "Region3",                  Region3)
    rawset(eR, "Ray",                      Ray)
    rawset(eR, "Axes",                     Axes)
    rawset(eR, "Faces",                    Faces)
    rawset(eR, "Font",                     Font)
    rawset(eR, "PhysicalProperties",       PhysicalProperties)
    rawset(eR, "PathWaypoint",             PathWaypoint)
    rawset(eR, "RaycastParams",            RaycastParams)
    rawset(eR, "OverlapParams",            OverlapParams)
    rawset(eR, "CatalogSearchParams",      CatalogSearchParams)
    rawset(eR, "DateTime",                 DateTime)
    rawset(eR, "Random",                   Random)
    -- ── task library ────────────────────────────────────────────────────────
    rawset(eR, "task",        task)
    rawset(eR, "wait",        wait)
    rawset(eR, "delay",       delay)
    rawset(eR, "spawn",       spawn)
    rawset(eR, "tick",        tick)
    rawset(eR, "time",        time)
    rawset(eR, "elapsedTime", elapsedTime)
    -- ── Lua standard library globals needed inside sandbox ──────────────────
    rawset(eR, "print",       print)
    rawset(eR, "warn",        warn)
    rawset(eR, "error",       i)
    rawset(eR, "assert",      assert)
    rawset(eR, "tostring",    m)
    rawset(eR, "tonumber",    tonumber)
    rawset(eR, "type",        j)
    rawset(eR, "typeof",      typeof)
    rawset(eR, "pairs",       D)
    rawset(eR, "ipairs",      E)
    rawset(eR, "next",        next)
    rawset(eR, "select",      select)
    rawset(eR, "unpack",      unpack)
    rawset(eR, "rawget",      rawget)
    rawset(eR, "rawset",      rawset)
    rawset(eR, "rawequal",    rawequal)
    rawset(eR, "rawlen",      rawlen)
    rawset(eR, "setmetatable",setmetatable)
    rawset(eR, "getmetatable",getmetatable)
    rawset(eR, "require",     require)
    rawset(eR, "loadstring",  loadstring)
    rawset(eR, "load",        loadstring)
    rawset(eR, "pcall",       function(fn, ...)
        local dg = {g(fn, ...)}
        if not dg[1] and type(dg[2]) == "string" and dg[2]:find("TIMEOUT_FORCED_BY_DUMPER", 1, true) then
            i(dg[2], 0)
        end
        return unpack(dg)
    end)
    rawset(eR, "xpcall", function(fn, handler, ...)
        local dg = {h(fn, handler, ...)}
        if not dg[1] and type(dg[2]) == "string" and dg[2]:find("TIMEOUT_FORCED_BY_DUMPER", 1, true) then
            i(dg[2], 0)
        end
        return unpack(dg)
    end)
    rawset(eR, "coroutine",   coroutine)
    rawset(eR, "string",      string)
    rawset(eR, "math",        math)
    rawset(eR, "os",          {clock = p.clock, time = p.time, difftime = os.difftime, date = os.date})
    rawset(eR, "io",          {})   -- blocked intentionally
    rawset(eR, "getfenv",     getfenv or function(fn) return eR end)
    rawset(eR, "setfenv",     setfenv or function(fn, env) return fn end)
    rawset(eR, "collectgarbage", function() return 0 end)
    rawset(eR, "_VERSION",    _VERSION)
    rawset(eR, "_G",          eR)
    -- ── syn / http aliases ──────────────────────────────────────────────────
    rawset(eR, "syn", {
        request = function(opts)
            local url = type(opts)=="table" and (opts.Url or opts.url) or tostring(opts or "")
            local body = _fetch_url(url) or "{}"
            return {Success=true, StatusCode=200, StatusMessage="OK", Headers={}, Body=body}
        end,
    ...
    -- Patch the game proxy inside eR to have a real HttpGet
    -- eUNC calls: loadstring(game:HttpGet(url))()
    -- game is our proxy object — add HttpGet directly to its property_store
    do
        local _eR_game_meta = a.getmetatable(game)
        if _eR_game_meta and _eR_game_meta.__index then
            -- The proxy already handles :HttpGet via bP.HttpGet which calls _fetch_url
            -- Just make sure the property_store entry is set too
        end
        -- Also expose a top-level game table in eR that has HttpGet
        local _eR_game_proxy = setmetatable({}, {
            __index = function(_, k)
                if k == "HttpGet" or k == "HttpGetAsync" then
                    return function(self_or_url, url_or_nil)
                        local url = type(self_or_url)=="string" and self_or_url or tostring(url_or_nil or "")
                        local body = _fetch_url(url)
                        if body and #body > 10 then return body end
                        return tostring(url)
                    end
                end
                -- Forward everything else to the real game proxy
                return game[k]
            end,
        ...
        rawset(eR, "game", _eR_game_proxy)
        rawset(eR, "Game", _eR_game_proxy)
    end
    -- LuraphContinue already in table literal above
    rawset(eR, "LuraphContinue", function() end)
    -- ── Common exploit-executor globals ─────────────────────────────────────
    -- Common exploit-executor globals.
    ...
    -- request / http_request: actually fetch URLs so eUNC test harness loads
    rawset(eR, "request",              function(o_)
        local url = type(o_) == "table" and (o_.Url or o_.url) or tostring(o_ or "")
        local body = _fetch_url(url) or "{}"
        return {Success=true, StatusCode=200, StatusMessage="OK", Headers={["Content-Type"]="text/plain"}, Body=body}
    end)
    rawset(eR, "http_request",         function(o_)
        local url = type(o_) == "table" and (o_.Url or o_.url) or tostring(o_ or "")
        local body = _fetch_url(url) or "{}"
        return {Success=true, StatusCode=200, StatusMessage="OK", Headers={["Content-Type"]="text/plain"}, Body=body}
    end)
    -- HttpGet on the game object inside sandbox (eUNC uses game:HttpGet)
    rawset(eR, "HttpGet", function(self, url)
        if type(self) == "string" then url = self end
        local body = _fetch_url(tostring(url or ""))
        if body then return body end
        return tostring(url or "")
    end)
    -- loadstring inside eR sandbox: fetch URL content and execute it
    rawset(eR, "loadstring", function(src, chunkname)
        if type(src) ~= "string" then return nil, "not a string" end
        -- If it looks like a URL, fetch it first
        if src:match("^https?://") then
            local body = _fetch_url(src)
            if body and #body > 10 then
                src = body
            else
                -- Can't fetch — return stub
                return function() return nil end
            end
        end
        -- Sanitise and compile
        if src:byte(1) ~= 0x1b then
            src = I(src)
        end
        local fn, err = e(src, chunkname or "loadstring")
        if fn then return fn end
        return nil, err
    end)
    rawset(eR, "load", rawget(eR, "loadstring"))
    ...
    do
        ...
        rawset(eR, "crypt", {
            ...
            encrypt        = function(s, key, iv, mode)
                -- XOR cipher stub — reversible for round-trip tests
    ...
    do
        local _vfs_files   = {}   -- path → content string
        local _vfs_folders = {}   -- path → true
    ...
    rawset(eR, "request",        function(opts)
        local url = type(opts) == "table" and (opts.Url or opts.url) or tostring(opts or "")
        local body = _fetch_url(url) or "{}"
        return {Success=true, StatusCode=200, StatusMessage="OK", Headers={["Content-Type"]="text/plain"}, Body=body}
    end)
    rawset(eR, "http_request",   function(opts)
        local url = type(opts) == "table" and (opts.Url or opts.url) or tostring(opts or "")
        local body = _fetch_url(url) or "{}"
        return {Success=true, StatusCode=200, StatusMessage="OK", Headers={["Content-Type"]="text/plain"}, Body=body}
    end)
