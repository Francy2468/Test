...
local r = {
    ...
    TIMEOUT_SECONDS = 30,  -- Internal limit; must be < DUMP_TIMEOUT in cat.py (35s) to allow cleanup
...
local function I(J)
    ...
    local function Q(R)
        ...
        R = R:gsub("\\", "")
        -- ── Eliminate 'local' keyword ────────────────────────────────────────────
        -- Converting all local declarations to global assignments bypasses Lua's
        -- hard 200-local-per-function limit that many heavily-obfuscated scripts
        -- exceed.  This is the "emit no local" guarantee: the preprocessed code
        -- that reaches load() contains zero 'local' keywords.
        -- Phase 1: local function → function  (keep as global function)
        R = R:gsub("([^%w_])local%s+(function[^%w_])", "%1%2")
        R = R:gsub("^local%s+(function[^%w_])", "%1")
        -- Phase 2: local VARLIST = EXPR → VARLIST = EXPR
        -- Uses a lazy match so multi-var lists (a, b, c = ...) are captured whole.
        R = R:gsub("([^%w_])local%s+([%a_][%w_ ,\t]-)%s*(=[^=])", function(b, v, e)
            return b .. (v:match("^(.-)%s*$") or v) .. " " .. e
        end)
        R = R:gsub("^local%s+([%a_][%w_ ,\t]-)%s*(=[^=])", function(v, e)
            return (v:match("^(.-)%s*$") or v) .. " " .. e
        end)
        -- Phase 3: bare local VARLIST (no = follows) → VARLIST = nil
        -- After phases 1 and 2 any remaining 'local' token is a bare declaration.
        R = R:gsub("([^%w_])local%s+([%a_][%w_ ,\t]*)", function(b, v)
            v = v:match("^(.-)%s*$") or v
            if v == "" then return b end
            return b .. v .. " = nil"
        end)
        R = R:gsub("^local%s+([%a_][%w_ ,\t]*)", function(v)
            v = v:match("^(.-)%s*$") or v
            if v == "" then return "" end
            return v .. " = nil"
        end)
        -- ── Anti-tamper / anti-debug patches ─────────────────────────────────────
        -- debug.sethook(): replace with a no-op call so statement structure is
        -- preserved and catlogger's own hook is not overridden or disrupted.
        R = R:gsub("debug%.sethook%s*%b()", "(function()end)()")
        -- debug.getinfo(): return an empty table instead of nil so that code which
        -- indexes the result (e.g. info.source) does not raise a nil-index error.
        R = R:gsub("debug%.getinfo%s*%b()", "{}")
        -- debug.traceback(): neutralize — used to dump stack as anti-tamper evidence.
        R = R:gsub("debug%.traceback%s*%b()", '""')
        -- debug.getlocal / getupvalue: neutralize — used to walk locals/upvalues.
        R = R:gsub("debug%.getlocal%s*%b()", "nil")
        R = R:gsub("debug%.getupvalue%s*%b()", "nil")
        return R
    end
